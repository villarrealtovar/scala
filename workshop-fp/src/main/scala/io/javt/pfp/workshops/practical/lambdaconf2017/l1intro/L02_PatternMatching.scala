...
object L02_PatternMatching {
  ...
  def gym2(month: String): Boolean = month match {
    case "June" => true
    case "July" => true
    case _ => false
  }
  ...
  case class Human2(life: Int, robot: Option[Robot]) extends Soldier

  def sendTo(soldier: Soldier): Place = soldier match {
    case Human2(life, Some(Robot(battery))) => Headquarters
    case Human2(life, None) => Headquarters
